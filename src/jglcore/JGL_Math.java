...
public final class JGL_Math {
	...
	/**
	 * Projects the vector on the plane according to its normal.
	 * @param v : the vector
	 * @param p : the plane normal
	 * @param overclip
	 * @param result : stores the result
	 */
	public static final void vector_projectOnPlane(JGL_3DVector v, JGL_3DVector p, float overclip, JGL_3DVector result) {
		float backoff = vector_dotProduct(v, p);
		if (overclip!=1f) {
			if (backoff<0f)
				backoff *= overclip;
			else
		...
		result.x = v.x - (p.x * backoff);
		result.y = v.y - (p.y * backoff);
		result.z = v.z - (p.z * backoff);
	}
	...
	public static final int plane_trianglePosition(JGL_3DPlane plane, JGL_3DTriangle triangle) {
		...
		if (Math.abs(d1)<=EPSILON && 
			...
			dot = vector_dotProduct(plane.normal, vect3);
			if (dot<0f)
				return 9;
			if (dot>0f)
				return 10;
	...
	/**
	 * Returns the specified mesh's position against the specified plane.<br>
	 * -1 : behind the plane<br>
	 *  0 : across the plane<br>
	 *  1 : before the plane<br>
	 * 
	...
	public static final int plane_meshPosition(JGL_3DPlane plane, JGL_3DMesh mesh) {
		...
		for (int i=0; i<mesh.getPoints().size(); i++) {
			...
			if (plane.behind(point)) rear += 1;
			if (plane.before(point)) front += 1;
		}
	...
	public static final int triangle_split(JGL_3DTriangle triangle, JGL_3DPlane plane, 
		...
		if(nbSplit == 1) {
			...
			result1.owner = result2.owner  = triangle.owner;
			return 2;
		}
		...
		if(nbSplit == 2) {
			...
			result1.owner = result2.owner = result3.owner = triangle.owner;
			return 3;
		}
		return 0;
	}
