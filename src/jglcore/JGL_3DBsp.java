...
public final class JGL_3DBsp implements JGL_3DStruct {
	...
	public void addFace(JGL_3DTriangle face) {
		...
		if (type!=NODE) {
			...
			((JGL_3DTriangle)faces.get(0)).owner = face.owner;
			facesSize = 1;
			return;
		}
	...
	public void getMesh(JGL_3DMesh result) {
		
		if (type==NODE)
			for (int i=0; i<faces.size(); i++)
				result.addFace((JGL_3DTriangle)faces.get(i));
		
		if(rear != null) rear.getMesh(result);
		if(front != null) front.getMesh(result);
	}
