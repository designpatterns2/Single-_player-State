...
public final class Mover_gravity implements Mover {
	...
	/** Minimum translation for gravity */
	public static float ZERO_G = 0.01f;
	...
	public void update() {
		...
		if (!jump || (jump && yCumul>=jumpHeight)) {
			...
			mov.y -= (gSpeed * JGL_Time.getTimePerFrame()) + ZERO_G;
		}
		else {
	...
	public boolean impactReaction(Trace trace, Tracable tracable) {
		...
		// Floor management
		if (trace.correction.normal.y > Util4Phys.FLOOR_NORMAL) {
			resetGravity();
			if (jump)
				jump = false;
		...
		// Ceiling management
		if (trace.correction.normal.y < Util4Phys.CEIL_NORMAL) {
			if (mov.y>0f)
				resetGravity();
			if (jump)
				yCumul = jumpHeight;
