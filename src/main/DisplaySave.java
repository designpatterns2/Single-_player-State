...
public class DisplaySave {
	...
	private static FSFileList fileList;
	...
	public static int display(Node rootNode, LightState lightState, 
		...
		Text2D create = my2dfont.createText("Create file", fontSize, 0);
		create.setLocalTranslation(new Vector3f(50, top - (files.size()*lineHeight), 0));
		create.setRenderQueueMode(Renderer.QUEUE_ORTHO);
        ZBufferState zbs = DisplaySystem.getDisplaySystem().getRenderer().createZBufferState();
        zbs.setFunction(ZBufferState.TestFunction.Always);
        create.setRenderState(zbs);
		...
		KeyBindingManager.getKeyBindingManager().set( "save", KeyInput.KEY_RETURN );
		...
		while (!back && !save) {
	        ...
	        if ( KeyBindingManager.getKeyBindingManager().isValidCommand("save", false ) ) {
	        	save = true;
	        }
		}
		...
		if (save) {
			if (index==files.size())
				fileList.createFile(World.getCurrentMap());
			else
				fileList.changeFile(index, World.getCurrentMap());
		}
