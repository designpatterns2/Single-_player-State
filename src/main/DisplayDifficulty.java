 ...
 *
 */
public class DisplayDifficulty {
	...
	private static Text2D hard;
	private static Text2D medium;
	private static Text2D easy;
	...
	public static void init() {
		...
		hard = my2dfont.createText("hard", 10f, 0);
		hard.setLocalTranslation(new Vector3f(width * 0.2f, height * 0.6f, 0f));
		hard.setLocalScale(height / 600f);
		hard.updateGeometricState( 0.0f, true );
		hard.updateRenderState();
		
		medium = my2dfont.createText("medium", 10f, 0);
		medium.setLocalTranslation(new Vector3f(width * 0.2f, height * 0.5f, 0f));
		medium.setLocalScale(height / 600f);
		medium.updateGeometricState( 0.0f, true );
		medium.updateRenderState();
		
		easy = my2dfont.createText("easy", 10f, 0);
		easy.setLocalTranslation(new Vector3f(width * 0.2f, height * 0.4f, 0f));
		easy.setLocalScale(height / 600f);
		easy.updateGeometricState( 0.0f, true );
		easy.updateRenderState();
	...
	public static int display(Node rootNode, LightState lightState, 
		...
		rootNode.attachChild(hard);
		rootNode.attachChild(medium);
		rootNode.attachChild(easy);
		...
		while (!start && !back) {
			...
			if ( KeyBindingManager.getKeyBindingManager().isValidCommand("action", false ) ) {
				if (action==0) {
					World.difficulty = World.HARD;
					start = true;
				}
				if (action==1) {
					World.difficulty = World.MEDIUM;
					start = true;
				}
				if (action==2) {
					World.difficulty = World.EASY;
					start = true;
				}
			}
		}
