...
public final class CollisionHeightMap implements CollisionNode {
	...
	private static Shape_sphere pPoint = new Shape_sphere(new JGL_3DVector(), 0.05f);
	 ...
	 * @param shininess
	 */
	public CollisionHeightMap(String id, JGL_3DVector pos, int width, int depth, float gap, JGL_3DMesh mesh) {
		...
		for (int i=0; i<p.size(); i++) {
			...
			w = (int)Math.abs(v.x / gap);
			d = (int)Math.abs(v.z / gap);
			if (w<width && d<depth)
				heights[w][d] = v.y;
		}
