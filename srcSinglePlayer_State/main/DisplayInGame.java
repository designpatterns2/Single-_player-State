...
public class DisplayInGame {
	...
	public static int display(Node rootNode, LightState lightState, 
        ...
        while (!Player.entity.isDead() && !World.map.isFinished() && !(World.mode==World.KINEMATIC) && !pause) {
	        ...
	        input.update( JGL_Time.getTimePerFrame() );
			...
			if (frustum != Player.frustumToReach) {
				if (frustum>Player.frustumToReach) {
					frustum -= Player.FRUSTUM_CHANGE * JGL_Time.getTimer();
					if (frustum<Player.frustumToReach)
						frustum = Player.frustumToReach;
				}
				if (frustum<Player.frustumToReach) {
					frustum += Player.FRUSTUM_CHANGE * JGL_Time.getTimer();
					if (frustum>Player.frustumToReach)
				...
				camera.setFrustumPerspective( frustum, (float) DisplaySystem.getDisplaySystem().getWidth()
			...
			if (frustum != Player.FRUSTUM_NORMAL)
