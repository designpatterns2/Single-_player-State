...
public class AlterMilkToJme  extends FormatConverter {
    ...
    public static void main(String[] args){
        ...
        new AlterMilkToJme().attemptFileConvert(args);
    }
    ...
    private void readMats() throws IOException {
        ...
        for (int i=0;i<nNumMaterials;i++){
            ...
            if (texFile.length()!=0){
            	...
            	texFile = texFile.replace('\\', ' ');
            	StringTokenizer st = new StringTokenizer(texFile);
            	String name = "";
            	while (st.hasMoreTokens()) name = st.nextToken();
            	Texture tempTex = LoadHelper.getTexture(name, true);
            	if (tempTex!=null) {
            		tempTex.setWrap(Texture.WrapMode.Repeat);
                	texState.setTexture(tempTex, 0);
                	if (texFile.contains("alpha")) {
            ...
            texFile=cutAtNull(tempChar);
            if (texFile.length()!=0){
                Texture tempTex = LoadHelper.getTexture(texFile, true);
                if( tempTex!=null) {
                	tempTex.setWrap(Texture.WrapMode.Repeat);
                	tempTex.setEnvironmentalMapMode(Texture.EnvironmentalMapMode.SphereMap);
                	texState.setTexture(tempTex, 1);
                }
            }
            if (texState.getNumberOfSetTextures()==0)
            	texState = null;
            else
            	texState.apply();
            
            applyStates(matState, as1, CS, texState, i);
        }
    }
     ...
     * states
     */
    private void applyStates(MaterialState matState, BlendState as1, CullState CS, TextureState texState,int index) {
        for (int i=0;i<finalNode.getQuantity();i++){
            if (materialIndexes[i]==index){
                ...
                if (as1!=null) {
                	((TriMesh)finalNode.getChild(i)).setRenderState(as1);
                	//((TriMesh)finalNode.getChild(i)).setRenderQueueMode(Renderer.QUEUE_TRANSPARENT);
                	((TriMesh)finalNode.getChild(i)).setTextureCombineMode(TextureCombineMode.Replace);
                }
                if (CS!=null) ((TriMesh)finalNode.getChild(i)).setRenderState(CS);
    ...
    private void readGroups() throws IOException {
        ...
        for (int i=0;i<nNumGroups;i++){
            ...
            //
            theMesh.setModelBound( new BoundingBox() );
            theMesh.updateModelBound();
            //
