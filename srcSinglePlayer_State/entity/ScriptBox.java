 ...
 *
 */
public final class ScriptBox implements Entity, Scriptable {
	 ...
	 */
	public ScriptBox(String id, Shape shape, Vector scripts, boolean checkpoint) {
	...
	public boolean touchReact(Entity entity, Trace trace) {
		...
		if (entity==Player.entity) {
			if (isCheckpoint) {
				if (World.difficulty==World.EASY) {
					World.map.setCheckpoint(this);
					Message.displayMessage("Checkpoint...", null, null, null);
				}
			}
			else
				executeScripts();
	...
	public Object clone() {
		return new ScriptBox(name, cshape, scr, isCheckpoint);
	}
